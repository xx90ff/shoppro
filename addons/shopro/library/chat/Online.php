...
class Online 
{
    ...
    public static function customerServiceOffline($client_id = null, $customerService)
    {
        ...
        // 获取客服在线状态
        $onlineStaus = self::customerServiceStatusById($customerService['id'], 'customer_service', ['client_id' => $client_id]);
    ...
    public static function allocatCustomerService($session_id, $user_id) {
        ...
        if ($last_customer_service) {
            ...
            if ($lastConnection) {
                // 获取上次客服信息
                $lastCustomerService = Online::customerServiceById($lastConnection['customer_service_id']);
                // 判断客服是否在线
                if ($lastCustomerService) {
                    if ($lastCustomerService['status'] == 'online') {
                        $currentCustomerService = $lastCustomerService->toArray();
                    }
                }
            }
        }
    ...
    {
        $except = $data['except'] ?? [];
        $historieLogs = Connection::with(['chat_user' => function ($query) use ($customerService) {
            $query->where('customer_service_id', $customerService['id']);
        }])->where('customer_service_id', $customerService['id'])->where('session_id', 'not in', $except)->group('session_id')->select();
    ...
    public static function messageList($linker, $data) {
        ...
        $messageList = ChatLog::with('identify')->where(function ($query) use ($session_id, $user_id) {
            $query->where(function ($query) use ($session_id) {
                $query->where('session_id', $session_id)
                    ->where('session_id', 'not null')
                    ->where('session_id', '<>', '');
